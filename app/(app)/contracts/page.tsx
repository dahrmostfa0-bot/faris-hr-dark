...
export default function ContractsPage() {
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from('contracts').insert({
      ...
      end_date: form.end_date || null, salary: Number(form.salary), status: 'active', terms: form.terms,
    });
    if (error) toast({ title: 'خطأ', description: error.message, variant: 'destructive' });
    else {
      toast({ title: 'تم إنشاء العقد' });
      setOpen(false);
      const { data } = await supabase.from('contracts').select('*, employee:employees(*)').order('created_at', { ascending: false });
  ...
  async function terminate(id: string) {
    ...
    setContracts((c) => c.map((x) => x.id === id ? { ...x, status: 'terminated' as ContractStatus } : x));
