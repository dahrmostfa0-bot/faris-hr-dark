...
export default function WarningsPage() {
  ...
  useEffect(() => {
    (async () => {
      const [wRes, eRes, cRes] = await Promise.all([
        supabase.from('warnings').select('*, employee:employees(*)').order('created_at', { ascending: false }),
        supabase.from('employees').select('id, full_name').order('full_name'),
        supabase.from('contracts').select('*, employee:employees(*)').order('end_date', { ascending: true }),
      ]);
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from('warnings').insert({
      ...
      issued_by: form.issued_by, labor_law_ref: form.labor_law_ref, notes: form.notes, status: 'active',
    });
    if (error) toast({ title: 'خطأ', description: error.message, variant: 'destructive' });
    else {
      toast({ title: 'تم تسجيل التنبيه' });
      setOpen(false);
      const { data } = await supabase.from('warnings').select('*, employee:employees(*)').order('created_at', { ascending: false });
      setWarnings((data ?? []) as Warning[]);
      // create notification for HR
      await supabase.from('notifications').insert({
        title: 'تنبيه جديد',
        body: `تم تسجيل ${WARNING_TYPE_LABELS[form.type]} للموظف ${employees.find((e) => e.id === form.employee_id)?.full_name ?? ''}`,
        type: 'warning',
        read: false,
      });
    }
