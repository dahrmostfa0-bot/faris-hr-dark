  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [empRes, depRes, cRes, lRes, prRes] = await Promise.all([
        ...
        supabase.from('contracts').select('id, type, end_date, employee:employees(full_name)').limit(10),
        supabase.from('leaves').select('id, type, start_date, employee:employees(full_name)').limit(10),
        supabase.from('payroll').select('id, net_salary, month, year, employee:employees(full_name)').limit(10),
      ]);
