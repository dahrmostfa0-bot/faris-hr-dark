...
export default function EmployeeDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [empRes, docRes, attRes, lvRes, prRes, tkRes, rvRes] = await Promise.all([
        supabase.from('employees').select('*, department:departments(*), position:positions(*), branch:branches(*), manager:employees!manager_id(*)').eq('id', id).maybeSingle(),
  ...
  if (!emp) return <div className="py-12 text-center">لم يتم العثور على الموظف</div>;
