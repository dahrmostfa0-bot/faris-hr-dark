...
import { Clock, LogIn, LogOut, Calendar, User, Filter, Upload, X, FileSpreadsheet, RefreshCw, Plus, Trash2, Check, UserX } from 'lucide-react';
...
export default function AttendancePage() {
  ...
  const [activeTab, setActiveTab] = useState<'attendance' | 'schedules' | 'management'>('attendance');
    ...
    (async () => {
      ...
      try {
        const { data: emps } = await supabase
          .from('employees')
          .select('id, full_name, employee_code, user_id, department:departments(*)')
          .eq('company_id', company.id)
          .order('full_name');
  ...
  async function quickCheckIn(emp: Employee) {
    const now = new Date();
    const todayStr = now.toISOString().slice(0, 10);

    if (!company || usingMock) {
      const exists = records.some((r) => r.employee_id === emp.id && r.date === todayStr);
      if (exists) { toast({ title: 'سبق تسجيل الحضور لهذا الموظف', variant: 'destructive' }); return; }
      ...
      toast({ title: 'تم تسجيل الحضور', description: emp.full_name });
      return;
    }
    ...
    const { data: existing } = await supabase
      ...
      .eq('company_id', company.id)
      .maybeSingle();
    if (existing) { toast({ title: 'سبق تسجيل الحضور لهذا الموظف', variant: 'destructive' }); return; }
    const isLate = now.getHours() > 8 || (now.getHours() === 8 && now.getMinutes() > 30);
  ...
  async function quickMarkAbsent(emp: Employee) {
    const now = new Date();
    const todayStr = now.toISOString().slice(0, 10);

    if (!company || usingMock) {
      setRecords((prev) => {
        const filtered = prev.filter((r) => !(r.employee_id === emp.id && r.date === todayStr));
        return [{
          id: `local-${Date.now()}`,
          employee_id: emp.id,
          date: todayStr,
      ...
      toast({ title: 'تم تسجيل الغياب', description: emp.full_name });
      return;
    }
    ...
    if (existing) {
      const { error } = await supabase.from('attendance')
        .update({ status: 'absent', check_in: null, check_out: null, work_hours: 0, late_minutes: 0 })
        .eq('id', existing.id);
      if (error) { toast({ title: 'خطأ', description: error.message, variant: 'destructive' }); return; }
    } else {
      const { error } = await supabase.from('attendance').insert({
  ...
  async function quickCheckOut(emp: Employee) {
    const now = new Date();
    const todayStr = now.toISOString().slice(0, 10);

    if (!company || usingMock) {
      const rec = records.find((r) => r.employee_id === emp.id && r.date === todayStr);
      if (!rec) { toast({ title: 'لم يسجل الموظف الحضور بعد', variant: 'destructive' }); return; }
      ...
      toast({ title: 'تم تسجيل الانصراف', description: emp.full_name });
      return;
    }
    ...
    const { data: existing } = await supabase
      ...
      .eq('company_id', company.id)
      .maybeSingle();
    if (!existing) { toast({ title: 'لم يسجل الموظف الحضور بعد', variant: 'destructive' }); return; }
  ...
  const todayMap = new Map<string, Attendance>();
  todayRecords.forEach((r) => todayMap.set(r.employee_id, r));
  ...
  return (
    <div className="space-y-6">
      ...
      <div role="tablist" aria-label="أقسام الحضور والانصراف" className="flex w-full max-w-2xl gap-2 rounded-xl border border-border/60 bg-muted/40 p-1">
        ...
        <button type="button" role="tab" aria-selected={activeTab === 'management'} onClick={() => setActiveTab('management')} className={`flex-1 rounded-lg px-4 py-2.5 text-sm font-medium transition-colors ${activeTab === 'management' ? 'bg-card text-primary shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
          إدارة الموظفين
        </button>
      </div>
      ...
      {activeTab === 'management' && (
        <Card>
          <CardHeader>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <CardTitle className="text-base">إدارة الموظفين والإجراءات المباشرة</CardTitle>
                <p className="mt-1 text-sm text-muted-foreground">رصد حالة كل موظف وتغييرها يدوياً بضغطة زر.</p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {employees.length === 0 ? (
              <div className="py-12 text-center text-muted-foreground">لا يوجد موظفون</div>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>وقت الحضور الفعلي</TableHead>
                      <TableHead>الحالة</TableHead>
                    ...
                    {employees.map((emp) => {
                      ...
                      return (
                        <TableRow key={emp.id} className="hover:bg-muted/40 transition-colors">
                          ...
                          <TableCell className="font-mono text-sm">{rec?.check_in ? formatTimeAr(rec.check_in) : '-'}</TableCell>
                          <TableCell>
                            {status ? (
                              <Badge variant={status === 'present' ? 'success' : status === 'late' ? 'warning' : status === 'absent' ? 'destructive' : 'secondary'}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1.5">
                              <Button size="sm" className="gap-1 bg-emerald-600 text-white hover:bg-emerald-700" disabled={!canManage || !!rec?.check_in} onClick={() => quickCheckIn(emp)}>
                                <Check className="h-3.5 w-3.5" /> حضور
                              </Button>
                              <Button size="sm" className="gap-1 bg-red-600 text-white hover:bg-red-700" disabled={!canManage || status === 'absent'} onClick={() => quickMarkAbsent(emp)}>
                                <UserX className="h-3.5 w-3.5" /> غياب
                              </Button>
                              <Button size="sm" className="gap-1 bg-blue-600 text-white hover:bg-blue-700" disabled={!canManage || !rec?.check_in || !!rec?.check_out} onClick={() => quickCheckOut(emp)}>
