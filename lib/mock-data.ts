...
function makeMockAttendance(): Attendance[] {
  ...
  MOCK_EMPLOYEES.slice(0, 7).forEach((emp, i) => {
    ...
    recs.push({
      ...
      employee_id: emp.id,
      date: today,
      check_in: s === 'absent' ? undefined : `${today}T0${8 + (s === 'late' ? 1 : 0)}:0${i % 6}0:00Z`,
      check_out: s === 'absent' ? undefined : `${today}T16:${30 + i}0:00Z`,
  ...
  for (let d = 1; d <= 7; d++) {
    ...
    MOCK_EMPLOYEES.slice(0, 6).forEach((emp, i) => {
      ...
      recs.push({
        ...
        employee_id: emp.id,
        date: ds,
        check_in: s === 'absent' ? undefined : `${ds}T08:${i * 5}0:00Z`,
        check_out: s === 'absent' ? undefined : `${ds}T16:${30 + i}0:00Z`,
        work_hours: s === 'absent' ? 0 : 8,
